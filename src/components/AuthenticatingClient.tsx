"use client";

import { useEffect, useState } from "react";

import { BrandIcon } from "@/components/Brand";
import { DotGridBackground } from "@/components/DotGridBackground";
import { loginApiHref, type AuthProvider } from "@/lib/auth/urls";

const DELAY_MS = 3000;

function GoogleMark() {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className="h-5 w-5">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

function MicrosoftMark() {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className="h-5 w-5">
      <path fill="#F25022" d="M2 2h9.5v9.5H2z" />
      <path fill="#7FBA00" d="M12.5 2H22v9.5h-9.5z" />
      <path fill="#00A4EF" d="M2 12.5h9.5V22H2z" />
      <path fill="#FFB900" d="M12.5 12.5H22V22h-9.5z" />
    </svg>
  );
}

const providerButtonClass =
  "flex h-11 w-full items-center justify-center gap-2.5 rounded-lg border border-border bg-surface text-sm font-medium text-foreground transition hover:bg-background";

/**
 * Sign-in entry: provider picker (Google / Microsoft), then a brief
 * “Authenticating…” pause before Internode PKCE with that provider.
 */
export function AuthenticatingClient({
  returnTo,
}: {
  returnTo?: string;
}) {
  const [provider, setProvider] = useState<AuthProvider | null>(null);
  const [secondsLeft, setSecondsLeft] = useState(Math.ceil(DELAY_MS / 1000));

  useEffect(() => {
    if (!provider) return;

    const started = Date.now();
    const tick = window.setInterval(() => {
      const remaining = Math.max(0, DELAY_MS - (Date.now() - started));
      setSecondsLeft(Math.ceil(remaining / 1000));
    }, 250);

    const redirectTimer = window.setTimeout(() => {
      window.location.assign(loginApiHref(returnTo, provider));
    }, DELAY_MS);

    return () => {
      window.clearInterval(tick);
      window.clearTimeout(redirectTimer);
    };
  }, [returnTo, provider]);

  if (!provider) {
    return (
      <main className="relative flex min-h-dvh items-center justify-center overflow-hidden px-4 py-10">
        <DotGridBackground />

        <div className="relative w-full max-w-sm rounded-2xl border border-border bg-surface p-8 shadow-sm">
          <h1 className="text-center text-[1.375rem] font-semibold tracking-tight text-foreground">
            Welcome to Skillbase
          </h1>
          <p className="mt-2 text-center text-sm text-muted">
            Continue to register or sign in.
          </p>

          <div className="mt-6 space-y-3">
            <button
              type="button"
              onClick={() => setProvider("google")}
              className={providerButtonClass}
            >
              <GoogleMark />
              Continue with Google
            </button>

            <button
              type="button"
              onClick={() => setProvider("microsoft")}
              className={providerButtonClass}
            >
              <MicrosoftMark />
              Continue with Microsoft
            </button>
          </div>
        </div>
      </main>
    );
  }

  return (
    <main className="flex min-h-dvh items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-surface p-8 text-center shadow-sm">
        <BrandIcon size={44} className="mx-auto mb-4 h-11 w-11" />
        <h1 className="text-lg font-semibold tracking-tight">Authenticating</h1>
        <p className="mt-2 text-sm text-muted">
          Checking for Internode credentials
          {secondsLeft > 1 ? "…" : ``}
        </p>
        <div
          className="mx-auto mt-6 h-1.5 w-40 overflow-hidden rounded-full bg-skeleton"
          aria-hidden
        >
          <div className="authenticating-bar h-full rounded-full bg-accent" />
        </div>
      </div>
    </main>
  );
}
