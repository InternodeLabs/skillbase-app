...
4. Feature gates (e.g. Edit) link to `/api/auth/login?returnTo=…` — **not** the
   `/login` interstitial. `/login` remains for callback error display.
...
src/
  app/
    ...
    login/page.tsx                 # error interstitial only for happy-path UX
